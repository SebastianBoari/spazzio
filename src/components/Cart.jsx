...
const Cart = () => {
  ...
  // Elimina un producto segun su id, (Filtrar me parecio mas facil que usar splice).
  const removeItem = (itemId) => {
    setCart(cart.filter((item) => item.id !== itemId));
  };
      
  // Restablece el carrito
  const clear = () => {
    setCart([]);
  };
          ...
          <article id='cart__sidePanel'>
            ...
            <div className='sidePanel__body'>
              ...
              <div className='body__buttons'>
                <button id='toBuy' onClick={() => clear()}>Finalizar Compra</button>
                <button id='toKeep'><Link to={"/catalogo"}>Seguir Comprando</Link></button>
                <button id='toClean' onClick={() => clear()}>Limpiar carrito</button>
              </div>
